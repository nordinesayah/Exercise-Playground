...
var deckSorted = Deck(mustBeSort: true)

print("\nDeck Description : \(deckSorted)\n")

var deckBlended = Deck(mustBeSort: false)

print("\nDeck Mixed : \(deckBlended)\n")

print("\nDraw method, draw the first card of cards and the place in outs (10 tests) : ")

for _ in 0...9 {
    _ = deckSorted.draw()
}

print("Actual Cards : \(deckSorted.cards)\n")
print("\nActual Outs : \(deckSorted.outs)\n")

print("\nFold method, place the card c in discards if it belongs well to outs : ")
let card5 = Card(c: Color.Spade, v: Value.Ace)
let card6 = Card(c: Color.Diamond, v: Value.Two)
deckSorted.fold(c: card5)
deckSorted.fold(c: card6)

print("Actual Outs : \(deckSorted.outs)\n")
print("\nActual Discards : \(deckSorted.discards)\n")
